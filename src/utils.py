"""Dataset loading and face preprocessing helpers."""
...
def load_fer2013(csv_path=DATA_PATH):
    """Load FER-2013 into train, validation and test numpy arrays."""
    ...
    for usage, split_name in {
        ...
        pixels = np.stack(
            [np.fromstring(value, sep=" ", dtype=np.float32) for value in subset["pixels"]]
        )
...
def preprocess_face(face):
    """Resize a grayscale face crop without repeating/truncating pixels."""
    face = np.asarray(face)
    if face.ndim != 2:
        raise ValueError(f"Expected a grayscale 2-D face crop, got shape {face.shape}")
    if face.size == 0:
        raise ValueError("Face crop must not be empty")
